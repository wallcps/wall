...
<div  id="nav-group">
    ...
    <ul>
        ...
        <?php if($data.$managetab['contact'] ==1 ){ ?><li><a class='<?php echo $tab=="contact"?"active":""; ?>' href="<?php echo $base_url; ?>group.php?gid=<?php echo $groupID; ?>&ptab=contact">Contact Us</a></li><?php }else{echo "";} ?>
        <?php if ($group_owner_id == $uid) { ?><li><a class='<?php echo $tab=="member_request"?"active":""; ?>' href="<?php echo $base_url; ?>group.php?gid=<?php echo $groupID; ?>&ptab=member_request">Request</a></li><?php }else{echo "";} ?>
   ...
   <?php if($session_uid){?>
    <ul class="text-right menu-right">
        ...
        echo '<button type="button" class="btn btn-xs btn-primary manage-tab" style="margin-right:5px;" data-toggle="modal" data-target="#exampleModal" data-whatever="@mdo">Manage tab</button>'; 
		}
			if($role_in_project>=1){
				echo '<button class="btn btn-xs btn-primary manage-tab" style="background-color:orange;">Admin</button>';
			 }else if($role_in_project==0){
				echo '<button class="btn btn-xs btn-primary manage-tab" style="background-color:orange;">Member</button>';
			 }else{
				 if($session_follow==0){
				 	echo '<button class="btn btn-xs btn-primary manage-tab" onclick="FollowProject('.$groupID.')">Follow</button>';	
				 }else{
					 echo '<button class="btn btn-xs btn-primary manage-tab" onclick="UnfollowProject('.$groupID.')">Unfollow</button>';
				 }
