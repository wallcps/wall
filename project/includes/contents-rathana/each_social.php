...
<div class="stbody" id="stbody<?php echo $omsg_id;?>" rel="<?php echo $time; ?>">
  ...
  <?php  
       $value = $Wall->Get_Project_Social_Need_Message_ID($sn_id,$msg_id);
               
                    $sn_id = $value['sn_id'];
                    $sn_msg_id = $value['msg_id'];
                    $sn_title = $value['msg_titl'];
...
<div class="sttext full">
    <div class="swamper-image"><img src="<?php echo 'images/'.$sn_image; ?>"></div>
    <div class="swamper-contain">
        <h4 style="padding:2px 14px;"><b>Titile</b>: <?php echo $sn_title; ?></h4>
        <h4 style="padding:2px 14px;">Keyword: <b><?php echo $sn_keyword; ?></b></h4>
        <p class="text-content"><?php echo html_entity_decode($sn_content);?></p>
    </div>
<?php
if($like_count>0)
{
...
<a href='<?php echo $base_url;?>group.php?gid=<?php echo $groupID;?>&ptab=contents&p=program&sn_id=<?php echo $sn_id;?>' class='share share_button icontext' original-title='<?php echo $share_status;?>' data='<?php echo $shareKey ?>'><?php echo $share_status;?></a>
<?php if($uid != $msg_uid) { ?>
